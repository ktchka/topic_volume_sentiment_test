...
import typer
from utils import load_json_data, setup_logging
...
class SimpleReportGenerator:
    ...
    def load_artifacts(
        self, artifacts_dir: str, classifications_file: str = "data/derived/classifications.json"
    ) -> dict[str, Any]:
        ...
        if not os.path.exists(classifications_file):
            classifications_file = os.path.join(
                os.path.dirname(artifacts_dir.rstrip("/")), "derived", "classifications.json"
            )
        ...
        if "classifications" in artifacts:
            classification_mode = (
                ...
                .get("metadata", {})
                .get("classification_mode", "openai")
            )
    ...
    def generate_executive_summary(self, artifacts: dict[str, Any]) -> str:
        ...
        if "classifications" in artifacts and "metadata" in artifacts["classifications"]:
            sentiment_accuracy = (
                artifacts["classifications"]["metadata"]
                .get("analysis", {})
                .get("sentiment_accuracy", 0.0)
            )
        ...
        if "classifications" in artifacts and "metadata" in artifacts["classifications"]:
            ...
            summary.append(
                f"- **Classification mode**: {metadata.get('classification_mode', 'unknown')}"
            )
            summary.append(
                f"- **Average topic confidence**: {analysis.get('avg_topic_confidence', 0):.3f}"
            )
            summary.append(
                f"- **Average sentiment confidence**: {analysis.get('avg_sentiment_confidence', 0):.3f}"
            )
            summary.append(
                f"- **Correct sentiments**: {analysis.get('correct_sentiments', 0)}/{analysis.get('total_sentiments', 0)}"
            )

            if analysis.get("merged_topics"):
                summary.append(
                    f"- **Merged topics into 'Other'**: {', '.join(analysis['merged_topics'])}"
                )
    ...
    def generate_volume_validation(self, artifacts: dict[str, Any]) -> str:
        ...
        section.append(
            f"**Max difference allowed**: {volume.get('max_difference_allowed', 0):.1f} percentage points\n"
        )
        ...
        if "topic_results" in volume:
            ...
            for topic, result in sorted(
                volume["topic_results"].items(),
                key=lambda x: x[1]["system_percentage"],
                reverse=True,
            ):
                status_icon = "PASS" if result.get("pass", False) else "FAIL"
                section.append(
                    f"| {topic} | {result['system_percentage']:.1f}% | "
    ...
    def generate_sentiment_analysis(self, artifacts: dict[str, Any]) -> str:
        ...
        for topic, data in sorted(
            per_topic.items(), key=lambda x: x[1]["total_tickets"], reverse=True
        ):
            ...
            for sentiment, count in sorted(
                ...
                percentage = (
                    (count / data["total_tickets"] * 100) if data["total_tickets"] > 0 else 0
                )
            ...
            section.append(
                f"- **Sentiment validation accuracy**: {validation.get('accuracy', 0):.1%}"
            )
    ...
    def generate_recommendations(self, artifacts: dict[str, Any]) -> str:
        ...
        if "volume" in artifacts:
            ...
            if not volume.get("overall_pass", False):
                failed_topics = [
                    topic
                    for topic, result in volume.get("topic_results", {}).items()
                ...
                if failed_topics:
                    ...
                    recommendations.append(
                        "The following topics have distribution differences exceeding the threshold:\n"
                    )
                    for topic in failed_topics:
                        recommendations.append(f"- **{topic}**")
                    recommendations.append("\n**Actions:**")
                    recommendations.append(
                        "- Review topic classification accuracy for these categories"
                    )
        ...
        if len(recommendations) == 1:  # Only has the header
            recommendations.append("No issues detected. All validation checks passed.\n")
...
def main(
    artifacts_dir: str = typer.Option(
        ..., "--artifacts", "-a", help="Directory containing validation artifacts"
    ),
    classifications_file: str = typer.Option(
        "data/derived/classifications.json",
        "--classifications",
        "-c",
        help="Classifications file path",
    ),
